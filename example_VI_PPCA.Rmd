 ...
 **REMARK** I think the previous result is wrong, as it is assumed that $\mathbb{E}_q (\Lambda'_{jh}\Lambda'_{jh'}) = \left[\mathbb{E}_q (\Lambda_{j})\mathbb{E}_q (\Lambda'_{j})+ \mathbb{V}_q (\Lambda_{j})\right]_{h,h'}$.
 
 Another way of solving is to notice is to rewrite (for a matrix $M$, denoting $M_j$ its $j-th$ column)
 
 \begin{align*}
 \log p(Y\vert \eta, \Lambda, \Sigma) &= \sum_{j = 1}^p \log p(Y_j\vert \eta, \Lambda_j, \Sigma) \\
 &= \frac{n}{2}\sum_{j = 1}^p \log \sigma^{-2}_j - \frac{1}{2}\sum_{j = 1}^p \sigma^{-2}_j\left(Y_j - \eta \Lambda_j'\right)^T\left(Y_j - \eta \Lambda_j'\right).
 \end{align*}
Now, we need to compute the expectation of the right hand term with respect to $\eta$ and $\Lambda_j$. We note that:
\begin{align*}
\left(Y_j - \eta \Lambda_j'\right)^T\left(Y_j - \eta \Lambda_j'\right) &= Y_j^TY_j - 2 Y_j^T\eta \Lambda_j' + \Lambda_j'^T \eta^T\eta \Lambda_j'\\
&=Y_j^TY_j - 2 Y_j^T\eta \Lambda_j' + \text{Tr}\left(\eta^T\eta \Lambda_j' \Lambda_j'^T \right).
\end{align*}
Thus, taking expectations, we have:
$$\mathbb{E}\left[ \left(Y_j - \eta \Lambda_j'\right)^T\left(Y_j - \eta \Lambda_j'\right) \right] = Y_j^TY_j - 2 Y_j^T\mathbb{E}[\eta] \mathbb{E}[\Lambda_j'] + \text{Tr}\left(\mathbb{E}[\eta^T\eta]\mathbb{E}\left[\Lambda_j' \Lambda_j'^T\right] \right)$$
...
Note that this matrix has to be computed only once for each $j$, and we conclude, using the fact that $\mathbb{E}\left[\Lambda_j' \Lambda_j'^T\right] = \mathbb{E}\left[\Lambda_j'\right]\mathbb{E}\left[\Lambda_j'^T\right] + \mathbb{V}\left[\Lambda_j'\right]$, we have that:
$$B^{\sigma_j}= b_\sigma+\frac{1}{2}\sum_{i=1}^n \left(Y_j^TY_j - 2 Y_j^T\mathbb{E}[\eta] \mathbb{E}[\Lambda_j'] + \text{Tr}\left(\mathbb{E}[\eta^T\eta]\left(\mathbb{E}\left[\Lambda_j'\right]\mathbb{E}\left[\Lambda_j'^T\right] + \mathbb{V}\left[\Lambda_j'\right]\right)\right)\right)$$
