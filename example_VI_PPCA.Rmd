...
Hence, 
$$\mathbb{E}_{q_{-\sigma^{-2}_j}}[f_{\sigma^{-2}_j}(Z_{\sigma^{-2}_j}, Z_{-\sigma^{-2}_j})] = \left(a_\sigma + \frac{n}{2} - 1\right)\log \sigma^{-2}_j - \left(b_{\sigma} + \frac{1}{2}\mathbb{E}_{q_{\eta,\Lambda_j}}\left[\left(Y^{j} - \eta \Lambda_j\right)^T\left(Y^{j} - \eta \Lambda_j\right)\right] \right)
$$
 ...
 **REMARK** I think the previous result is wrong, as it is assumed that $\mathbb{E}_q (\Lambda'_{jh}\Lambda'_{jh'}) = \left[\mathbb{E}_q (\Lambda_{j})\mathbb{E}_q (\Lambda'_{j})+ \mathbb{V}_q (\Lambda_{j})\right]_{h,h'}$.
 ...
 \begin{align*}
 \log p(Y\vert \eta, \Lambda, \Sigma) &= \sum_{j = 1}^p \log p(Y^{j}\vert \eta, \Lambda_j, \Sigma) \\
 &= \frac{n}{2}\sum_{j = 1}^p \log \sigma^{-2}_j - \frac{1}{2}\sum_{j = 1}^p \sigma^{-2}_j\left(Y^{j} - \eta \Lambda_j'\right)^T\left(Y^{j} - \eta \Lambda_j'\right).
 \end{align*}
Now, we need to compute the expectation of the right hand term with respect to $\eta$ and $\Lambda_j$. We note that:
\begin{align*}
\left(Y^{j} - \eta \Lambda_j'\right)^T\left(Y^{j} - \eta \Lambda_j'\right) &= (Y^{j})^TY^{j} - 2 (Y^{j})^T\eta \Lambda_j' + \Lambda_j'^T \eta^T\eta \Lambda_j'\\
&=(Y^{j})^TY^{j} - 2 (Y^{j})^T\eta \Lambda_j' + \text{Tr}\left(\eta^T\eta \Lambda_j' \Lambda_j'^T \right).
\end{align*}
...
$$B^{\sigma_j}= b_\sigma+\frac{1}{2}\sum_{i=1}^n \left((Y^{j})^TY^{j} - 2 (Y^{j})^T\mathbb{E}[\eta] \mathbb{E}[\Lambda_j'] + \text{Tr}\left(\mathbb{E}[\eta^T\eta]\left(\mathbb{E}\left[\Lambda_j'\right]\mathbb{E}\left[\Lambda_j'^T\right] + \mathbb{V}\left[\Lambda_j'\right]\right)\right)\right)$$
