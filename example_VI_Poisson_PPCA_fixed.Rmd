...
At each step of the CAVI algorithm, $n \times p$ calls to the R optim subroutine performs the optimisation of a target function with two arguments :$M^{Z_{i,j}$ and $V^{Z_{i,j}$.
$$ Y_{i,j}e^{M^{Z_{i,j}}+\frac{V^{Z_{i,j}}}{2}}-0.5 \times \mathbb{E}_{q}\left(Z_{i,j} - \eta_i \Lambda_j-X_i\beta_j\right)^2 -\frac{\log\vert {V^{Z_{i,j}}} \vert}{2}$$
...
$$ Y_{i,j}e^{M+\frac{V}{2}}-0.5 \times((M^2+V)-2\times M \times \mathbb{E}_{q}\left( \eta_i \Lambda_j+X_i\beta_j\right)) -\frac{\log\vert {V} \vert}{2}$$
Wrapping up, the partial ELBO function to maximise is:
$$ Y_{i,j}e^{M+\frac{V}{2}}-0.5 M^2-0.5V+ M \times \left( M^{\eta_i} M^{\Lambda_j}+X_iM^{\beta_j}\right) -\frac{\log\vert V \vert}{2}$$
