...
res_known_eta <- get_gibbs_sample(data_ = Y, n_steps = n_steps, k_tilde = 2,
                           Eta_true = Eta_true)
...
res_known_lambda <- get_gibbs_sample(data_ = Y, n_steps = n_steps, k_tilde = 2,
...
select_eta_rows <- sample(1:nrow(Y), size = 10, replace = FALSE) %>% 
  sort()
...
ggplot(Eta_hat_df) + 
  aes(x = Estimate) +
  geom_density() +
  facet_wrap(~Parameter, nrow = length(select_eta_rows), labeller = label_parsed) +
...
## Estimation of taus
tau_hat_df <- tibble(estimate= as.vector(result$taus),
                     parameter=rep(paste0("tau",1:nrow(result$taus)),ncol(result$taus)))
tau_hat_df %>% ggplot(aes(y=log(estimate),color=parameter))+geom_boxplot()+labs(y="log(precision)")
