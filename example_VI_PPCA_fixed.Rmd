...
Bhattacharya and Dunson consider the following linear normal model with a gamma process to shrink the variances of the columns of the latent matrix $\lambda$. Relying on this setting, we include a fixed effect (due to $f=1:qk$ forcing conditions from known explanatary variates with intensity $X_{f,i}$ on site $i$ interveining with coefficient $\beta_{j,f}$ onto species $j=1:p$) in addition to the random ones (governed by the unknown sparse loading matrix $\Lambda$):
\begin{align*}
Y_i &
...
In the code, $Y$ is a known $n\times p$ matrix, $X$ is a known $n\times q_k$ matrix while the unknown $\Lambda$ will be a $p \times q$ matrix, $\beta$ will be a $p \times q_k$ matrix and $\eta$ a $n\times q$ matrix.
...
1\leq j \leq p, 
\end{align*}
where $\eta_i$ denotes the $i$-th row of yhe $n\times q$ matrix $\eta$,  $\Lambda_j$ denotes the $j$-th row of the $p\times q$ matrix $\Lambda$,and $\beta_j$ denotes the $j$-th row of the $p\times qk$ matrix $\beta$. In the following, all vectors are treated as column vectors.
...
$$\log\left([\beta|\Sigma]\right)= 0.5\times q_k\sum_j^p log(\sigma_j^{-2}) -0.5\times p\sum_f^{q_k} log(c_f)\\-0.5 \sum_{j,f}^{p,q_k} c_f^{-1} \sigma_j^{-2}\beta_{jf}^2$$
Returning to vectors ($\beta_j$ is the $j$th row of matrix $\beta$):

$$\log\left([\beta_j|\Sigma]\right)= 0.5\times q_k \times log(\sigma_j^{-2}) -0.5\times \sigma_j^{-2} \times Tr(diag(c_f^{-1})\beta_j \beta_j^T)$$
